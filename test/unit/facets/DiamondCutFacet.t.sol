...
contract DiamondCutFacetTest is Test {
    ...
    function test_initialize_ShouldSetSupportedInterfaces() public {
        ...
        assertEq(
            MoreVaultsStorageHelper.getSupportedInterface(address(facet), type(IDiamondCut).interfaceId),
            true,
    ...
    function test_facetName_ShouldReturnCorrectName() public view {
        assertEq(IDiamondCut(facet).facetName(), "DiamondCutFacet", "Facet name should be correct");
    }
    ...
    function test_onFacetRemoval_ShouldDisableInterface() public {
        ...
        assertFalse(MoreVaultsStorageHelper.getSupportedInterface(address(facet), type(IDiamondCut).interfaceId));
    }
    ...
    function test_diamondCut_ShouldAddNewFacet() public {
        ...
        vm.mockCall(
            REGISTRY,
            abi.encodeWithSelector(IMoreVaultsRegistry.isFacetAllowed.selector, mockFacetAddress),
            abi.encode(true)
        );
        ...
        address[] memory facets = MoreVaultsStorageHelper.getFacetAddresses(facet);
        ...
        bytes4[] memory selectors = MoreVaultsStorageHelper.getFacetFunctionSelectors(facet, mockFacetAddress);
    ...
    function test_diamondCut_ShouldReplaceExistingFacet() public {
        ...
        vm.mockCall(
            ...
            abi.encodeWithSelector(IVaultsFactory.link.selector, mockFacetAddress),
            ""
        );
        ...
        vm.mockCall(
            REGISTRY,
            abi.encodeWithSelector(IMoreVaultsRegistry.isFacetAllowed.selector, newTestFacet),
            abi.encode(true)
        );
        vm.mockCall(
            REGISTRY,
        ...
        vm.mockCall(
            address(0), // unassigned factory
            abi.encodeWithSelector(IVaultsFactory.unlink.selector, mockFacetAddress),
            ""
        );
        ...
        // Verify function selectors
        bytes4[] memory selectors = MoreVaultsStorageHelper.getFacetFunctionSelectors(facet, newTestFacet);
        assertEq(selectors.length, 1, "Should have one selector");
        assertEq(selectors[0], TEST_SELECTOR, "Should have correct selector");
    }
    ...
    function test_diamondCut_ShouldRevertWhenNotCurator() public {
        ...
        vm.prank(user);
        vm.expectRevert(abi.encodeWithSelector(AccessControlLib.UnauthorizedAccess.selector));
        IDiamondCut(facet).diamondCut(cuts);
    }
